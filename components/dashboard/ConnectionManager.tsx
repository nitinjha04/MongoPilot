...
export const ConnectionManager: React.FC<ConnectionManagerProps> = ({ onConnectionSelect }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const connectionName = formData.name.trim();
      const connectionUri = formData.uri;
      ...
      const added = useStore
        .getState()
        .connections.find((c) => c.name === connectionName && c.uri === connectionUri);

      if (added) {
        updateConnection(added.id, {
          status: 'connected',
  ...
  const handleConnect = async (connectionId: string): Promise<boolean> => {
    ...
    try {
      ...
      toast.error(result.error || `Failed to connect to ${connection.name}`);
      return false;
    } catch (error) {
      updateConnection(connectionId, {
        status: 'error',
